...
1. Read the user's input or infer from conversation context
2. Read `phases/00-orientation.md` and follow its instructions for domain detection and complexity assessment
3. Detect domain by reading `adapters/{domain}.md`:
...
### Phase 1-2: Inline Conversation (no delegation)

Read `phases/01-concrete-case.md` then `phases/02-friction-cost.md`.
Conduct these directly — no delegation needed. These are conversational.

### Phase 3+: Expert Delegation (delegate tool)

At Phase 3 and beyond, spawn parallel expert agents using the `delegate` tool:

```
For each needed expert:
  1. Use delegate tool to spawn a sub-agent
  2. Prompt: "Read experts/{expert}.md then analyze the following from your specialized perspective: [session context summary]"
...
Read phase files only when needed: `phases/{NN}-{name}.md`
...
2. Read appropriate template by mode:
...
3. Read adapter: `adapters/{domain}.md` for domain-specific output
4. Read scoring rubric: `scoring/rubric.md`
